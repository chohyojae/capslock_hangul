//! 빌드 스크립트: 버전 정보(VERSIONINFO) 리소스를 DLL 에 임베드한다.
//! → 파일 속성 "자세히" 탭에 파일 설명·파일 버전·제품 이름·저작권이 표시된다.
//!
//! 본체(루트 build.rs)와 동일한 방식이지만 FILETYPE 이 VFT_DLL(0x2)이고 manifest 는 넣지 않는다
//! (DLL 은 실행 권한 manifest 가 무의미). embed-resource 는 Windows SDK 의 rc.exe 를 사용한다.
